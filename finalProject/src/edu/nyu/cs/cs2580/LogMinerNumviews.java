	...
	public void computeNumView() {
		try {
			...
			while ((content = br.readLine()) != null) {
				...
				String tmpName = results[1].replaceAll("[^a-zA-Z0-9]", "_");
				String resultName;
				
				int count = 0;
				if (!tmpName.matches("[_]")) {
					resultName = tmpName;
				} else {
					resultName = tmpName + count;
				}
				//System.out.println(results[0] + "\t" + results[1] + "\t" + results[2]);
				// only handle the file in corpus
				if (numViewValues.containsKey(resultName)) {
					if (results.length == 3 && results[2].matches("[0-9]*")){
						int newValue = numViewValues.get(resultName) + Integer.parseInt(results[2]);
						numViewValues.put(resultName, newValue);
					}
				}
			}
